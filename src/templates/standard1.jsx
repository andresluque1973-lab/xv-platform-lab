...
// ─────────────────────────────────────────────────────────────────────────────
// 🎨 Constantes visuales — no son datos de cliente, se quedan aquí
// ─────────────────────────────────────────────────────────────────────────────
const ROSE       = "#c4848a";
...
function generateCode() {
  return "XV-" + Math.random().toString(36).slice(2, 6).toUpperCase();
}
        ...
        <div className="space-y-4">
          <div className="p-4" style={{background:"rgba(0,0,0,0.22)",border:"1px solid rgba(255,255,255,0.1)"}}>
            ...
            <p className="text-lg tracking-widest select-all"
              style={{fontFamily:"'Playfair Display',Georgia,serif",color:"#fff",letterSpacing:"0.12em"}}>
              {config.regalo.alias}
            </p>
          </div>
          {config.regalo.cvu && (
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center overflow-hidden"
      ...
      <div className="text-center px-8 relative z-10" style={{transform:vis?"translateY(0)":"translateY(40px)",opacity:vis?1:0,transition:"all 1.2s cubic-bezier(0.16,1,0.3,1)"}}>
        ...
        <h1 className="mb-3" style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:"clamp(3rem,10vw,6rem)",fontWeight:400,letterSpacing:"-0.02em",lineHeight:1,color:"#fff",textShadow:"0 2px 30px rgba(0,0,0,0.18)"}}>
          {config.titulo}
        </h1>
        <div className="mb-2" style={{fontFamily:"'Dancing Script',cursive,Georgia,serif",fontSize:"clamp(2.5rem,8vw,5rem)",fontWeight:700,color:"#fff",textShadow:"0 0 50px rgba(255,220,225,0.5)"}}>
          {config.nombre}
        </div>
        <p className="text-sm tracking-widest uppercase mb-12" style={{fontFamily:"'Cormorant Garamond',Georgia,serif",color:"rgba(255,240,242,0.7)"}}>
          {config.subtitulo}
        </p>
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-end overflow-hidden"
      ...
      <div className="relative z-10 text-center pb-0 pt-32 px-4" style={{transform:loaded?"translateY(0)":"translateY(60px)",opacity:loaded?1:0,transition:"all 1.4s cubic-bezier(0.16,1,0.3,1) 0.3s"}}>
        <p className="text-xs tracking-[0.5em] uppercase mb-6" style={{fontFamily:"'Cormorant Garamond',Georgia,serif",color:"rgba(255,240,242,0.8)"}}>
          {config.fecha_display}
        </p>
        <h2 style={{fontFamily:"'Dancing Script',cursive,Georgia,serif",fontSize:"clamp(4rem,15vw,9rem)",fontWeight:700,lineHeight:0.9,color:"#fff",textShadow:"0 0 80px rgba(255,200,215,0.55),0 4px 20px rgba(0,0,0,0.12)"}}>
          {config.nombre}
        </h2>
        <p className="mt-6 text-sm tracking-[0.2em]" style={{fontFamily:"'Cormorant Garamond',Georgia,serif",color:"rgba(255,240,242,0.75)"}}>
          Tu presencia en esta noche nunca la olvidaré
        </p>
      </div>
...
function MusicSection({ config, playing, onToggle }) {
  return (
    <section className="py-20 px-4 text-center" style={{background:"#5a2830",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
      ...
      <p className="text-xs mb-6" style={{fontFamily:"'Cormorant Garamond',Georgia,serif",color:"rgba(255,200,210,0.28)",letterSpacing:"0.1em"}}>
...
function GuestBlock({ num, data, onChange, errors }) {
  const inp = (field) => ({
    ...
    borderBottom:errors?.[field]?"1px solid rgba(255,120,120,0.8)":"1px solid rgba(255,200,210,0.3)",
    outline:"none",width:"100%",padding:"8px 0",fontSize:"0.95rem",transition:"border-color 0.3s",
  });
  return (
    <div className="p-6 space-y-5" style={{background:"rgba(0,0,0,0.15)",border:"1px solid rgba(255,255,255,0.15)"}}>
      <p className="text-xs tracking-widest uppercase" style={{color:"rgba(255,255,255,0.4)",fontFamily:"'Cormorant Garamond',Georgia,serif"}}>Invitado {num}</p>
      <div>
        <input value={data.name} onChange={e => onChange("name",e.target.value)} placeholder="Nombre" style={inp("name")}/>
      ...
      <div>
        <input value={data.surname} onChange={e => onChange("surname",e.target.value)} placeholder="Apellido" style={inp("surname")}/>
...
function ConfirmSection({ config }) {
  ...
  const validate = () => {
    const newErrors = guests.map(g => ({ name: !g.name.trim(), surname: !g.surname.trim() }));
    setErrors(newErrors);
    return newErrors.every(e => !e.name && !e.surname);
  };

  const handleSubmit = async () => {
    if (!validate()) { setErrMsg("Por favor completá los campos requeridos de todos los invitados."); return; }
    setErrMsg("");
    setStatus("loading");
    ...
    } catch { /* no-cors siempre resuelve */ }
    setCode(uniqueCode);
    setStatus("success");
  };

  const sel = {
    fontFamily:"'Cormorant Garamond',Georgia,serif",color:"#fff",background:"transparent",
    borderBottom:"1px solid rgba(255,200,210,0.3)",outline:"none",width:"100%",padding:"8px 0",fontSize:"0.95rem",
  };

  return (
    <section className="py-32 px-4" style={{background:"linear-gradient(180deg,#c4848a 0%,#9e6068 100%)"}}>
      <div className="max-w-md mx-auto">
        <div className="text-center mb-12">
          <div className="text-4xl mb-6" style={{color:"rgba(255,255,255,0.7)"}}>✉</div>
          <p className="text-xs tracking-[0.5em] uppercase mb-4" style={{fontFamily:"'Cormorant Garamond',Georgia,serif",color:"rgba(255,255,255,0.6)"}}>Confirmá tu asistencia</p>
        ...
        {status==="success" ? (
          <div className="text-center py-16">
            ...
            <p style={{fontFamily:"'Playfair Display',Georgia,serif",fontWeight:400,fontSize:"1.3rem",color:"#fff"}}>¡Gracias, {guests[0].name}!</p>
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs tracking-widest uppercase block mb-2" style={{color:"rgba(255,255,255,0.5)"}}>Número de personas</label>
              <select value={guestCount} onChange={e => setGuestCount(Number(e.target.value))} style={sel}>
                {Array.from({length:MAX_GUESTS},(_,i)=>i+1).map(n => (
                  <option key={n} value={n} style={{background:"#9e6068"}}>{n} {n===1?"persona":"personas"}</option>
                ))}
              </select>
            </div>
            {guests.map((g,i) => (
              <GuestBlock key={i} num={i+1} data={g} onChange={(field,val) => updateGuest(i,field,val)} errors={errors[i]}/>
            ))}
            {errMsg && (
              <p className="text-xs text-center py-2" style={{fontFamily:"'Cormorant Garamond',Georgia,serif",color:"rgba(255,160,160,0.9)"}}>{errMsg}</p>
            )}
            <button onClick={handleSubmit} disabled={status==="loading"}
              className="w-full py-4 text-xs tracking-[0.3em] uppercase transition-colors duration-300"
              style={{fontFamily:"'Cormorant Garamond',Georgia,serif",background:status==="loading"?"rgba(0,0,0,0.45)":"rgba(0,0,0,0.28)",color:"#fff",border:"1px solid rgba(255,255,255,0.2)",cursor:status==="loading"?"not-allowed":"pointer",opacity:status==="loading"?0.7:1}}
...
function Footer({ config }) {
  return (
    <footer className="py-16 text-center" style={{background:"#3d1820",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
      <div className="mb-4" style={{fontFamily:"'Dancing Script',cursive,Georgia,serif",fontSize:"2rem",color:ROSE_LIGHT}}>
        {config.nombre}
      </div>
      <p className="text-xs tracking-[0.4em] uppercase" style={{fontFamily:"'Cormorant Garamond',Georgia,serif",color:"rgba(255,200,210,0.3)"}}>
        {config.dia_semana} {config.fecha_display} · {config.hora}
      </p>
    </footer>
  );
}
  ...
  if (error) {
    return (
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:"#1a1a1a",color:"#e8b4b8",fontFamily:"Georgia,serif",textAlign:"center",padding:"2rem"}}>
        <div>
          <p style={{fontSize:"1.2rem",marginBottom:"0.5rem"}}>No se pudo cargar la invitación.</p>
          <p style={{fontSize:"0.8rem",opacity:0.5}}>{error}</p>
        </div>
      </div>
    );
  }
